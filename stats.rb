...
class Stats
  ...
  def process_coders(coders)
    ...
    @zjuers = coders.select {|coder| coder.has_records?}
  end
  ...
  def process_rounds(rounds)
    ...
    records.each do |record|
      if @highests.empty? || record.new_rating > @highests.last.new_rating
        @highests << record
      end
    end
    ...
    @div_winner = records_div.collect do |records|
      records.select {|record| record.rank == 1}
    end
    @onsites = records.select {|record| record.onsite?}
  end

  def gen_rank_html(file_name, title, lists, headers, attributes, rank)
...
END
        ...
        lists.each do |item|
          f.write("	<tr>\n")
          if rank
            f.write("		<td><span>#{item[0]}</span></td>\n")
            item = item[1]
          end
  ...
  def gen_div_winner_html(file_name, title, titles, lists, headers, attributes)
    File.open("HTML/stats/#{file_name}.html", "w") do |f|
      wrap_html_body(f) do
        write_header(f, title, ["../stats.css"])
        lists.zip(titles).each do |list, title|
          f.write(<<END
<div class="rankTable">
<h2>#{title}</h2>
<table border="0" cellspacing="0" cellpadding="0"><tbody>
...
END
  ...
  def gen_zjuers_html(file_name, title, lists)
    File.open("HTML/stats/#{file_name}.html", "w") do |f|
      wrap_html_body(f) do
        write_header(f, title, ["../stats.css"])
        f.write(<<END
...
END
        ...
        lists.each_with_index do |coder, index|
          f.write("	<tr>\n") if index % 8 == 0
          f.write("		<td>#{coder.handle_html_inner_link}</td>\n")
          f.write("	</tr>\n") if index % 8 == 7
        end
  ...
  def gen_htmls()
    ...
    gen_rank_html("chasucrate", "Top 20 challenge success rate (with at least 20 challenges)", @top_cha_suc_rate,
                  ...
                  [:handle_html_inner_link, :cha_suc_html, :cha_total_html,
                    :cha_suc_rate_html, :events_html], true)
    gen_rank_html("chapointstotal", "Top 20 challenge points in total (with at least 20 challenges)", @top_cha_points,
                  ...
                  [:handle_html_inner_link, :cha_points_html, :cha_suc_html,
                    :cha_fail_html, :events_html], true)
    gen_rank_html("chapointsavg", "Top 20 challenge points in average (with at least 20 challenges)", @top_cha_points_avg,
                  ...
                  [:handle_html_inner_link, :cha_points_avg_html,
                    :cha_points_html, :events_html], true)
    ...
    gen_rank_html("chapointsin1event", "Top 20 challenge points in one event",
                  ...
                  [:handle_html_inner_link, :round_coder_html, :div_html,
                    :cha_points_html, :cha_suc_html, :cha_fail_html], true)
    ...
    gen_rank_html("ratingrecord", "First to achieve the highest ratings",
                  @highests.reverse, ["Handle", "Date", "Event", "Rating"],
                  [:handle_html_inner_link, :date_html, :round_coder_html,
                    :new_rating_html], false)
    gen_rank_html("onsite", "All Onsite ZJUers", @onsites.reverse,
                  ["Handle", "Event", "Date", "Rank"],
                  [:handle_html_inner_link, :round_coder_html, :date_html,
                    :rank_html], false)
    gen_div_winner_html("divwinners", "All division winners",
                        ["All division 1 winners", "All division 2 winners"],
                        @div_winner.collect(&:reverse),
                        ["Handle", "Event", "Date"],
                        [:handle_html_inner_link, :round_coder_html, :date_html]
                        )
    gen_zjuers_html("zjuers", "All ZJUers (with at least one event)",
               @zjuers.sort_by {|c| c.id.to_i})
